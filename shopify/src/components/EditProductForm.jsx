...
import { updateSellerProduct } from "../api/sellerApi";
...
function EditProductForm({ product, onSuccess, onCancel }) {
  ...
  useEffect(() => {
    if (product) {
      ...
      setSellingPrice(product.sellingPrice?.toString() || product.price?.toString() || "");
      setDiscountPercent(product.discountPercent?.toString() || "");
      setGstIncluded(product.gstIncluded?.toString() || "true");
      setMinimumOrderQuantity(product.minimumOrderQuantity?.toString() || "1");
      ...
      setStockAvailability(product.stockAvailability || "ready");
      setPackageWeight(product.packageWeight?.toString() || "");
      setPackageLength(product.packageLength?.toString() || "");
      setPackageWidth(product.packageWidth?.toString() || "");
      setPackageHeight(product.packageHeight?.toString() || "");
      setPickupAddress(product.pickupAddress || "");
      setDeliveryMethod(product.deliveryMethod || "fulfilled");
      setGstNumber(product.gstNumber || "");
      setHsnCode(product.hsnCode || "");
      setBrandAuthorized(product.brandAuthorized?.toString() || "true");
      setTrademarkVerified(product.trademarkVerified?.toString() || "true");
      setReturnPolicy(product.returnPolicy || "");
      setReplacementAvailable(product.replacementAvailable?.toString() || "true");
      ...
      // Handle compliance certificates
      if (product.complianceCertificates) {
        if (typeof product.complianceCertificates === "string") {
          setComplianceCertificates(product.complianceCertificates);
        } else {
          setComplianceCertificates(JSON.stringify(product.complianceCertificates));
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (sellingPrice) {
        ...
        formData.append("productPrice", sellingPrice);
      }
      ...
      const result = await updateSellerProduct(product.id, formData, token);
          ...
          <div className="form-group">
            ...
            {existingImages.length > 0 && (
              <div className="existing-images">
                <p style={{ marginTop: "1rem", fontWeight: 600 }}>Current Images:</p>
